/*
   ...
   Description : Takes in two general simple polygons(not necessarily
                 convex) and computes the area of their intersection.
...
inline double cross(const V &a, const V &b, const V &c, const V &d)
{
...
int leftRight(const V &a, const V &b, const V &p)
{
...
int isInsideTriangle(V &a, V &b, V &c, V &p)
{
...
// triangulation. (n >= 3)
void triangulate(list<V> &P, vector<Triangle> &T)
{
    list<V>::iterator a, b, c, q;
    Triangle t;
    T.clear();
    if (P.size() < 3)
        return;
...
int isectLineSegs(V &a, V &b, V &c, V &d, V &p)
{
    double r, s, dn, n1, n2;
...
double areaPoly(vector<V> &P)
{
    double area = 0.0;
    vector<V>::iterator p, q;
...
bool radialLessThan(const V &a, const V &b)
{
    return (ORDER == leftRight(P0, a, b));
}

double isectAreaTriangles(Triangle &a, Triangle &b)///Two Triangle intersect area
{
    vector<V> P;
    vector<V>::iterator s, e, ne;
    V p;
    Triangle T[2] = { a, b };
    int i, j, r, t, u, v;
    double area = 0;
...
double isectAreaGpoly(list<V> &P, list<V> &Q)
{
    double area = 0.0;
    vector<Triangle> S, T;
...
int main()
{
    ...
    for (j = 0; j < 2; j++)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ///Give CW/ACW point of the polygon depend on ORDER value
            scanf("%lf %lf", &p.x, &p.y);
            P[j].push_back(p);
        }

        printf("Polygon %d:\n", j+1);
        for (list<V>::iterator q=P[j].begin(); q != P[j].end(); q++)
            printf(" (%.2f, %.2f)\n", q->x, q->y);
        printf("\n");
    }
