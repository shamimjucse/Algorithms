/* 3D Geometry: Geometry Primitives
   ...
   Notes: 	All the operators (+,-,*,/) are mandatory, but
		...
		missing.
	Covered:
	+,-,*(with scaller),/(with scaller)
	dot(a,b)
	cross(a,b)
	lng2(a):squared len
	lng(a):len
	closest_pt_iline(a,b,p):closest point on (a,b) line from point p
	closest_pt_seg(a,b,p):closest point on (a,b) segment from point p
	closest_pt_plane(norm,a,p):closest point on plane (norm,a) from poitn p
	closest_pt_plane(a,b,c,p):closest point on plane (a,b,c) from poitn p
	sphere_iline_isect(certre,radius,a,b,&p,&q):see comments
	[***]to_plane(a,b,c,p):convert 3d to 2d ,see comment
	line_line_dist(a,b,c,d):[good function :)]
	closest_approach(a,b,c,d,p,q):[very good function]
	on_iline(a,b,p):[check whether point p is online or offline :P]
	on_seg(a,b,p):[similar to online]
	plane_iline_isect(norm,origin,a,b,*p):plane line intersection
*/
...
Point closest_pt_iline(Point a, Point b, Point p) //Closest point on the Line from given point
{
    double along = dot(b-a,p-a)/lng2(b-a);
    return (b-a)*along + a;
}
Point closest_pt_seg(Point a, Point b, Point p) //Closest point on the Segment from given point
{
    double along;
    if (lng2(b-a) < EPS)
        return a;
...
int sphere_iline_isect(Point c, double r, Point a, Point b, Point *p, Point *q)
{
